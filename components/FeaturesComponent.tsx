...
export default function Feature({
  ...
  return (
    <>
      {/* For Mobile view */}
      <section className="w-full flex justify-center px-[var(--spacing-20)] py-[var(--spacing-32)] bg-[var(--colors-black-5)] rounded-[var(--spacing-10)] border-[0.5px] border-[var(--colors-black-10)] h-[302px] lg:hidden md:hidden">
        <div className="w-full flex flex-col items-end gap-[var(--spacing-12)]">
          <>
            <FeatureLoader size={20} />
          </>
          <div className="w-full flex flex-col items-start px-[var(--spacing-10)] gap-[4px]">
            <h3 className="text-3xl font-bold">{title}</h3>
            <p className="text-lg font-normal text-[var(--colors-black-70)]">
              {description}
            </p>
          </div>
          <div className="flex items-center gap-[var(--spacing-10)]">
            <IconButton
              ...
              bgColor="bg-[var(--base-black)]"
            />

            <IconButton
              icon={<ChevronRight fill={"white"} size={24} />}
              color="text-[var(--base-white)]"
              bgColor="bg-[var(--colors-black-80)]"
            />
          </div>
        </div>
      </section>
